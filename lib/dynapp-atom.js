...
/*
      ...
      if (serverFilesHasBeenEdited) {
        ...
        myNotification.onclick = () => {
          for(index in oldList){
            dataitemsHandeler.uploadFile(oldList[index], index)

          }
        }
      } else {
        for(index in oldList){
          promises.push(dataitemsHandeler.uploadFile(oldList[index], index))
      ...
      if (serverSourcesHasBeenEdited) {
        let myNotification = new Notification("Det finns en nyare version av projektet", {
          body: 'Klicka här om du vill skriva över filerna på serven'
        })
        myNotification.onclick = () => {
          for(index in oldSources){
            datasourceHandeler.uploadSource(index, oldSources[index])
          }
        }
      } else {
        for(index in oldSources){
          promises.push(datasourceHandeler.uploadSource(index, oldSources[index]))
        }
      }
*/
  ...
  createConfig() {
    ...
    var filepath = atom.project.getPaths()[0]
    fs.writeFile(filepath + '/dynappconfig.json', JSON.stringify(dynappconfig, null, 4));
    ...
    try{
      ...
      console.log("data-source-items/data-objects already exists")
    }
