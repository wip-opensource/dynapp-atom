...
const hidePassword = (editor) => {
  var keyElems = editor.element.querySelectorAll('.syntax--key');
  for (let i = 0, l = keyElems.length; i < l; i++) {
    let element = keyElems[i];
    if (element.textContent == '"password"') {
      valueElem = element.nextElementSibling.nextElementSibling;
      if (valueElem) {
        var password = valueElem.childNodes[1].textContent;
        var hiddenPassword = '*'.repeat(password.length);
        valueElem.childNodes[1].textContent = hiddenPassword;
      }
      break;
    }
  }
...
module.exports = {
  ...
  activate(state) {
    ...
    // Register listener to hide password in dynappconfig.json
    this.subscriptions.add(atom.workspace.observeActiveTextEditor(editor => {
      if (editor && editor.getFileName() == 'dynappconfig.json') {
        hidePassword(editor);
        this.subscriptions.add(
          editor.buffer.onDidStopChanging(() => hidePassword(editor))
        );
      }
    }));
  },
...
`
  ...
  async createConfig() {
    ...
    if (treeView) {
      // Get project root folder element
      elem = treeView.entryForPath(config.projectPath());
      elem.click();
      elem.click();
    }
    atom.notifications.addInfo('Config file created', {});
  },
...
`;
    ...
    if (!fileExists) {
      ...
      if (messages) {
        atom.notifications.addInfo('Ignore file created', {});
      }
    } else if (messages) {
