...
module.exports = {
  ...
  newDataSourceItem() {
    let dialog = new InputDialog({
      labelText: 'Filename',
      async callback (inputFileName) {
        let projectPath = config.projectPath();
        let fileName = path.parse(inputFileName).name;
        let python = `
          #!/usr/bin/env python
          import json
        ...
        await Promise.all([
          fs.writeFile(path.join(projectPath, 'data-source-items', fileName + '.meta.json'), JSON.stringify(meta), 'utf8'),
