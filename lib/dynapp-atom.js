...
const dataSourceHandler = require('./../DataSourceHandler.js');
const dataItemsHandler = require('./../DataItemsHandler.js');
const fileListHandler = require('./../FileListHandler.js');
const dataObjectsHandeler = require('./../DataObjectsHandler.js');*/
...
/*var addListeners = function() {
  dataItemsHandler.addOnDeleteListener();
  dataSourceHandler.addOnDeleteListener();
  dataObjectsHandeler.addOnDeleteListener();

  // TODO: Prototype, superdirty
  let watcher = chokidar.watch(atom.project.getPaths()[0] + '/data-items/webapp/dist/*');
  watcher.on('change', function(filePath) {
    if (!fs.existsSync(atom.project.getPaths()[0] + '/.files.json')){
      return;
    }

    let fileObject = JSON.parse(fs.readFileSync(atom.project.getPaths()[0] + '/.files.json', 'utf8'));

    var relPath = path.relative(atom.project.getPaths()[0] + '/data-items/', filePath);

    if(fileObject.fileList[relPath] != undefined){
      console.log("making dirty")
      fileObject.fileList[relPath]['dirty'] = true;
      fs.writeFile(atom.project.getPaths()[0] + '/.files.json', JSON.stringify(fileObject, null, 4));
    }
  });
        ...
        if(!fs.existsSync(atom.project.getPaths()[0] + '/.files.json')){
          ...
          if(editedFiles[cb.path] === undefined){
            ...
            if(cb.path.indexOf('data-source-items') != -1){
              ...
              dataItemsHandler.postFile(path.basename(cb.path)).then(function(){
                atom.notifications.addInfo("Filen: " + path.basename(cb.path) + " finns nu på serven", {})
              })
...
export default {
  ...
  newSource() {
    var dialog = new InputDialog({
      ...
      callback: function(text){
        ...
        //dataSourceHandler.postFile(path.basename(text)).then(function(){
          //atom.notifications.addInfo("Filen: " + path.basename(text) + " finns nu på serven", {})
        //})
        var filepath = atom.project.getPaths()[0]
        var fileObject = JSON.parse(fs.readFileSync(filepath + '/.files.json', 'utf8'))
  ...
  newItem() {
    var dialog = new InputDialog({
      ...
      callback: function(text) {
        ...
        if (text.indexOf(path.sep) != -1) {
          mkdirp(text, path.join(filepath, 'data-items'));
        }

        fs.writeFile(filepath + '/data-items/' + text, 'write here', 'binary', (err) => {
          if (err) throw err;

          var filepath = atom.project.getPaths()[0];
          var fileObject = JSON.parse(fs.readFileSync(filepath + '/.files.json', 'utf8'));
          fileObject['fileList'][text] = {
            dirty:true
          };
  ...
  uploadItems() {
    atom.notifications.addInfo("Filerna sparas", {})
    ...
    fileListHandler.getFileList().then(function(newList) {
      ...
      for (listItem in oldList) {
        promises.push(dataItemsHandler.uploadFile(oldList[listItem], listItem));
      }

      // for data sources
      for (source in oldSources) {
        promises.push(dataSourceHandler.uploadSource(source, oldSources[source]));
      }
      ...
      if(!serverFilesHasBeenEdited){
        Promise.all(promises).then(function(cb){
          var filepath = atom.project.getPaths()[0]
          var files = JSON.parse(fs.readFileSync(filepath + '/.files.json', 'utf8'))
          for(index in cb){
            file = cb[index]
            if(file != undefined){
              delete files[file.list][file.name].dirty
            }
          }

          atom.notifications.addInfo("Filerna är nu uppladdade", {})
  ...
  download() {
    ...
    fileListHandler.getFileList().then(function(result) {
      var files = validateFilesToDownload(result);
      var filepath = atom.project.getPaths()[0];
      var promises = [];
      // if the project is downloaded
      if (fs.existsSync(filepath + '/.files.json')) {
        var localFileObject = JSON.parse(fs.readFileSync(filepath + '/.files.json', 'utf8'));

        // Data items
        for (object in files.fileList) {
          if (object in localFileObject.fileList) {
            promises.push(dataItemsHandler.downloadFile(object, localFileObject.fileList[object]['etag']))
          } else {
            /* TODO: We really want to work with undefined values? */
            promises.push(dataItemsHandler.downloadFile(object, undefined));
          }
        }

        for (object in files.dataSources) {
          if (object in localFileObject.dataSources) {
            promises.push(dataSourceHandler.downloadSource(object, localFileObject.dataSources[object]['etag']))
          } else{
            /* TODO: We really want to work with undefined values? */
            promises.push(dataSourceHandler.downloadSource(object, undefined))
          }
        }

        for(object in files.dataObjects){
          if (object in localFileObject.dataObjects) {
      ...
      // first time for this project
        ...
        for (file in files.fileList) {
          promises.push(dataItemsHandler.downloadFile(file))
        }
        for(source in files.dataSources){
          promises.push(dataSourceHandler.downloadSource(source))
        }
    ...
    Promise.all(promises).then(function(cb){
      for(obj in cb){
        ...
        if(localFileObject[obj.list][obj.fileName] != undefined){
          ...
          delete localFileObject[obj.list][obj.fileName].dirty
